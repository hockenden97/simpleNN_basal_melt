...
echo "Comparing variable distribtions"
variable='melt_m_ice_per_y'
job_type='COMPARE_'
echo ${variable}

# Where to find the python script to run the job on 
path_python=/bettik/ockendeh/SCRIPTS/simpleNN_basal_melt/AIAI_scripts_notebooks/COMPARE_variables.py
...
path_jobname=$path_${job_type}${variable}
echo "Running these variables"

path_sh_file=${path_jobid}${job_type}${variable}
path_sh_local=${path_local}${job_type}${variable}
...
python -u $path_python ${variable} 2>&1 
