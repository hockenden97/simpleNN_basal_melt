...
# Load filepaths
filepaths = glob.glob(filepath_pierre + '*gridT*.nc')
# Work out what parameters are needed for the files 
simulations = []
years = []
months = []
...
for i in range(len(filepaths_to_run)):
    year = int(filepaths_to_run[i].split(os.path.sep)[9].split('_')[1].split('.')[0].split('m')[0].split('y')[1])
    month = (filepaths_to_run[i].split(os.path.sep)[9].split('_')[1].split('.')[0].split('m')[1])
    simulation = filepaths_to_run[i].split(os.path.sep)[9].split('_')[0].split('-')[1]
    years.append(year)
    months.append(month)
    simulations.append(simulation)
print('Filepaths prepped,', len(filepaths_to_run), 'to run.')
...
#for i in range(2):
    ...
    extract_useful_variables_monthly(nemo_run, year_of_interest, month_of_interest, filepath_pierre, filepath_masks, filepath_distances, 
                                      filepath_nn_input, verbose = 0)
    # Record end time
    end_time = time.time()
    # Calculate elapsed time
    elapsed_time = end_time - start_time
    print('Currently processing:', years[i], months[i],'                           ', end = '\r')
    print(f" {years[i]} {int(months[i]):02d} : {elapsed_time:.4f} seconds                         ")
